var info ={
 ...
 var name_pokemon= window.prompt("enter name for the infomation adout pokemon ");
  var i=0;
  for ( i=0 ; i<155; i++){
  var chacke_name_of_pokemon=info.pokemon[i].name
    if (chacke_name_of_pokemon==name_pokemon){
    //  alert(info.pokemon[i].weaknesses)
      alert('id is '+info.pokemon[i].id + '  hight is' +info.pokemon[i].height +'  weight is ' +info.pokemon[i].weight
            +'   cady is '+info.pokemon[i].candy+'   camdy count '+info.pokemon[i].candy_count+'   egg  '+info.pokemon[i].egg
            +'  spawn chance is ' +info.pokemon[i].spawn_chance+'  multipliers is '+ info.pokemon[i].multipliers
            )
      alert(info.pokemon[i].img)
    }
  }
  var name_pokemon_evo= window.prompt("enter name for the next evolution pokemon ");
   i=0;
  for( i=0 ; i<155 ; i++){
    var chacke_name_of_pokemon=info.pokemon[i].name
    if (chacke_name_of_pokemon==name_pokemon_evo) {
        alert(info.pokemon[i].next_evolution[0].name+info.pokemon[i].next_evolution[1].name)
    }  
  }
  var weeknes_of_p= window.prompt("enter name for the next evolution pokemon ");
 i=0;
 for (i=0 ; i< 155 ;i++){
   for(j=0; j<15 ; j++){
    var check_weeknes=info.pokemon[i].weaknesses[j]
    if( check_weeknes==weeknes_of_p){
      alert(info.pokemon[i].name)
    }
   }
